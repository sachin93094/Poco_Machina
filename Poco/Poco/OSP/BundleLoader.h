...
public:
    ...
    BundleLoader(CodeCache& codeCache, BundleFactory::Ptr pBundleFactory, BundleContextFactory::Ptr pBundleContextFactory, bool autoUpdateCodeCache = true, std::string loggingAppID = "");
		/// Creates the BundleLoader.
    ...
    #ifdef IS_ARM_CAS
    void getAppActivationObjectsList(std::vector<std::pair<std::string, std::string>> &appActivationObjects);

    void updateActivationState(std::string& name, char& state);
    #endif
...
private:
    ...
    #ifdef IS_ARM_CAS
    std::vector<std::pair<std::string, std::string>> _appActivationObjectsList;
    bool isPersistencyRead;

    bool readPersistency(unsigned char** buffer, int *buffer_size);
    bool parseAppsActivationStates(unsigned char* buffer, int buffer_size);

    void readWatchedBundleScomoId(std::string&);
    #endif
};
