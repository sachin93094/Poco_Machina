...
#include "Poco/AbstractEventMonitor.h"
...
public:
	...
	void notify(const void* pSender, TArgs& args, const bool& monitored = false, const long& timeout = 10000)
		...
		/// to the caller.
	{
		...
		if(monitored){
			...
			AbstractEventMonitor monitor(timeout);
		...
		/// the execution is aborted and the exception is propagated to the caller.
	{
    	...
    	void notifyAsync(const void* pSender, const TArgs& args, const long& timeout)
        	/// Sends a notification to all registered delegates. The order is
        	/// determined by the TStrategy. This method is not blocking and will
        	/// immediately return. The delegates are invoked in a seperate thread.
        	/// Call activeResult.wait() to wait until the notification has ended.
        	...
        	/// the execution is aborted and the exception is propagated to the caller.
    	{
        	NotifyAsyncParams params(pSender, args, timeout);
        	{
			typename TMutex::ScopedLock lock(_mutex);
...
protected:
	struct NotifyAsyncParams
	{
		...
		NotifyAsyncParams(const void* pSend, const TArgs& a, long timeout = 0):ptrStrat(), pSender(pSend), args(a), enabled(true), timeout(timeout)
	...
	TArgs executeAsyncImpl(const NotifyAsyncParams& par)
	{
		...
		if(params.timeout > 0){
			Thread monitorThread;
			AbstractEventMonitor monitor(params.timeout);
			monitorThread.start(monitor);
...
public:
	...
	void notify(const void* pSender, const bool& monitored = false, const long& timeout = 10000)
		/// Sends a notification to all registered delegates. The order is 
		/// determined by the TStrategy. This method is blocking. While executing,
		/// the list of delegates may be modified. These changes don't
		/// influence the current active notifications but are activated with
		...
		/// the execution is aborted and the exception is propagated to the caller.
	{
    	...
    	void notifyAsync(const void* pSender, const long& timeout)
        	/// Sends a notification to all registered delegates. The order is
        	/// determined by the TStrategy. This method is not blocking and will
        	/// immediately return. The delegates are invoked in a seperate thread.
        	/// Call activeResult.wait() to wait until the notification has ended.
        	...
        	/// the execution is aborted and the exception is propagated to the caller.
    	{
        	NotifyAsyncParams params(pSender, timeout);
        	{
            		typename TMutex::ScopedLock lock(_mutex);
...
protected:
	struct NotifyAsyncParams
	{
		...
		NotifyAsyncParams(const void* pSend, long timeout = 0):ptrStrat(), pSender(pSend), enabled(true), timeout(timeout)
			/// Default constructor reduces the need for TArgs to have an empty constructor, only copy constructor is needed.
		{
		}
	};
